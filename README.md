...
-- 第三个参数`ctx`表示 上下文窗口的使用情况。

-- 第四个参数显示配置 deepseek api 后的当前账户余额（CNY）。
...
The second is the name of the currently used model.

The third parameter ctx represents the context window usage.

The fourth parameter displays the current account balance (CNY) after configuring the deepseek api.

Currently it is only possible to use configuration on windows.
